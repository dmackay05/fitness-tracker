...
function dsDecodeSets(ex){
  ...
  var loadDelim = loadsRaw.indexOf('|')>=0 ? '|' : (loadsRaw.indexOf('/')>=0 ? '/' : null);
  var loads=loadDelim ? loadsRaw.split(loadDelim) : reps.map(function(){return loadsRaw;});
...
function trkTryRestore(){
  try{
    ...
    var rideBtn=document.getElementById("trk-ride"), walkBtn=document.getElementById("trk-walk"), ruckBtn=document.getElementById("trk-ruck"), runBtn=document.getElementById("trk-run");
    if(rideBtn&&walkBtn&&ruckBtn){ rideBtn.classList.toggle("sel",trk.activity==="ride"); walkBtn.classList.toggle("sel",trk.activity==="walk"); ruckBtn.classList.toggle("sel",trk.activity==="ruck"); if(runBtn) runBtn.classList.toggle("sel",trk.activity==="run"); }
    var distEl=document.getElementById("trk-dist"); if(distEl) distEl.style.color = trk.activity==="ride" ? "#fb923c" : (trk.activity==="ruck" ? "#c4b5fd" : (trk.activity==="run" ? "#f472b6" : "#5eead4"));
    ...
    var ivlPanelR=document.getElementById('ivl-panel'); if(ivlPanelR) ivlPanelR.style.display = (trk.activity==="ride"||trk.activity==="walk"||trk.activity==="run") ? "block" : "none";
...
function setTrackActivity(a){
  ...
  var runBtn=document.getElementById("trk-run"); if(runBtn) runBtn.classList.toggle("sel",a==="run");
  document.getElementById("trk-dist").style.color = a==="ride" ? "#fb923c" : (a==="ruck" ? "#c4b5fd" : (a==="run" ? "#f472b6" : "#5eead4"));
...
function ivlSetPreset(key){
  ...
  if(trk.active && (trk.activity==='ride'||trk.activity==='walk'||trk.activity==='run') && !trk.paused) ivlBegin();
  ivlRenderStatus();
}
...
function trkStart(){
  ...
  if((trk.activity==='ride'||trk.activity==='walk'||trk.activity==='run') && ivl.on) ivlBegin(); else { ivl.phase=null; }
  trkPersist();
}
function trkOnErr(e){
...
function trkOnPos(p){
  ...
  if(trk.lastPt){
    ...
    var spd=d/dt; // m/s
    var cap   = trk.activity==="ride" ? 25 : (trk.activity==="run" ? 8.5 : 6);   // ~56mph ride / ~19mph sprint ceiling / ~13mph walk ceiling
...
function trkFinish(){
  ...
  trkReset();
  ivlOff();
  var _lbl=trk.activity==="ride"?"ride":(trk.activity==="ruck"?"ruck":(trk.activity==="run"?"run":"walk"));
...
function trkSaveManual(){
  ...
  var zoneMsg="";
  if(avgHr && (trk.activity==="ride"||trk.activity==="run")){ var z=dsHrZone(avgHr); if(z) zoneMsg=" \u2014 "+z.zone+" ("+Math.round(z.pct)+"% max HR)"; }
  var _lbl=trk.activity==="ride"?"ride":(trk.activity==="ruck"?"ruck":(trk.activity==="run"?"run":"walk"));
...
function trkCommit(miles,dur,source,avgHr,intervals){
  ...
  if(trk.activity==="ride"){
    ...
    var runMin = dur>0 ? dur : Math.round(miles*11); // fallback est. (~11 min/mi easy jog) if only distance was logged
...
function dsFlowFinishAll(){
  ...
  // Just flip the "done" badge — don't also add the flat Wednesday Yoga Flow
  // summary exercise (dsAddEx via tgYogaToggle). Each pose was already logged
  // individually with its own real calories/actualSecs, so adding the summary
  // here double-counted calories and, since that summary entry never carries
  // actualSecs, dragged total logged time down to a bogus flat 90s estimate.
  try{ if(typeof egSetDone==='function' && !((typeof egIsDone==='function')&&egIsDone(TG_YOGA_ID))) egSetDone(TG_YOGA_ID,true); }catch(e){}
  try{ tgYogaRefresh(); }catch(e){}
  dsRender(); renderAll();
}
...
// Replaces the KB Halo as the shoulder-specific warm-up on upper days — same
// intent (prep the overhead/behind-the-head path before pressing, given the
// right shoulder history), but a band gives real, graduated resistance
// through the range instead of one fixed light weight the whole way.
var DS_WARMUP_BANDSHOULDER={id:'warmup-bandshoulder',name:'Banded Shoulder Pass-Through',demo:'shouldercar',slot:'Warm-up',target:'Shoulders · Overhead Mobility',equip:'Tube 10–20 lb (wide grip)',rx:'10–12 reps',cal:12,cue:'Wide overhand grip on the band, arms straight — raise it overhead and back behind you, then return to the front. Keep the circle tight on the right side, same overhead path where your right shoulder history shows up.',log:'setsreps',sets:1,
  setup:'Hold the band with a wide, overhand grip — wider than shoulder-width, the tighter the grip the harder this gets, so start wide. Arms straight the whole time, raise the band up and overhead, continuing back behind you as far as comfortable, then reverse back to the front. 10-12 slow reps. Widen your grip if the shoulders round or the lower back arches to compensate.',
// Short embedded pre-hab, folded straight into warm-up on the days that need it most,
// rather than living only in the optional Extras > Mobility tab. Abbreviated versions of
// the full CARs + PAILs/RAILs protocol (~90s vs. the ~4 min Extras version) — same idea
// (own the joint through a slow circle, then a brief end-range contraction), just enough
// to prime the joint before it gets loaded. The full versions (mob-hip, mob-shoulder)
// are still in Extras for anyone who wants the deeper 4-min version too.
  variants:[{name:'PVC/Broomstick Pass-Through',equip:'Broomstick or dowel',rx:'10–12 reps',cue:'No band needed — same wide-grip pass-through pattern with a stick instead. Useful on a day even light band tension feels like too much before you\'re warm.',demo:'shouldercar'}]};
// RAMP-protocol "Raise" phase — the mobility drills below cover Mobilize
// and (via the per-exercise ramp sets) Potentiate, but nothing was actually
// raising heart rate/tissue temperature first. A short bout of easy general
...
var DS_WARMUP_HIPCARS={id:'wu-hip-cars',name:'Hips — Quick CARs + PAILs/RAILs',demo:'hipcircle',slot:'Warm-up · Prehab',target:'Hip Joint · SI Joint Prep',equip:'Floor / wall',rx:'~90 sec',cal:12,cue:'2–3 slow hip circles each direction (CARs) — own the joint through its full range. Then one quick ~60–70% PAILs/RAILs contraction in a comfortable hip fold, just enough to prime it before squats/hinges load it. Full 4-min version still in Extras > Mobility if you want more.',log:'time',secs:90};
var DS_WARMUP_SHOULDERCARS={id:'wu-shoulder-cars',name:'Shoulders — Quick CARs + PAILs/RAILs',demo:'shouldercar',slot:'Warm-up · Prehab',target:'Shoulder Joint',equip:'Wall / doorway',rx:'~90 sec',cal:10,cue:'2–3 slow overhead shoulder circles each direction (CARs) — own the joint through its full range, extra important given the right shoulder history. Then one quick ~60–70% PAILs/RAILs contraction at an end-range reach before pressing. Full 4-min version still in Extras > Mobility if you want more.',log:'time',secs:90};
...
var DS_SESSIONS={
    ...
    moves:[DS_WARMUP_RAISE,DS_WARMUP_ARMCIRCLE,DS_WARMUP_HIPFLOW9,DS_WARMUP_BANDSHOULDER,DS_WARMUP_SHOULDERCARS,
      {id:'mon-pushup',name:'Banded Push-ups',pat:'pushup',slot:'Push · Horizontal Push',target:'Chest · Triceps',equip:'Tube band or bodyweight',rx:'3–4×10–15',cal:35,cue:'Chest to floor, elbows 45° back — push the floor away explosively. Wrists stay stacked directly under the shoulders — if the left wrist complains from full extension, try push-up handles/blocks or a slight fist grip to keep it neutral.',demo:'pushup',log:'setsreps',sets:4,
        variants:[{name:'Wall Push-up',equip:'Bodyweight only',rx:'3×12–15',cue:'Hands on the wall, feet back — lean in and press away. Easiest regression, start here if floor push-ups aren\'t happening yet',demo:'wallpushup'},
                  ...
                  {name:'Super Band Push-up',equip:'Clench 35–75 lb band across upper back',rx:'3×8–12',cue:'Band under both palms, draped across the upper back — explode off the floor against the tension',demo:'pushup'},
      ...
      {id:'mon-lateral',name:'Lateral Raise',demo:'lateralraise',slot:'Push · Side Delts',target:'Side Delts',equip:'Tube 10 → 30 lb',rx:'3×12–15',cal:25,cue:'Second side-delt session of the week — same movement as Thursday, added because side delts were sitting well under everything else in the plan. Lead with elbows, not hands — pour water from a pitcher.',log:'setsreps',sets:3,
      ...
      {id:'thu-facepull',name:'Rear Delts / Face Pull',slot:'Pull · Rear Delts',target:'Rear Delts · Traps',equip:'Tube 10–20 → 50 lb',rx:'3–4×15–20',cal:30,cue:'Pull to your temples, elbows high — thumbs point behind you at the finish',demo:'facepull',log:'setsreps',sets:4,
        variants:[{name:'Cross-Body Rear Delt Fly',equip:'Tube 10–20 lb · chest-height anchor',rx:'3×12–15/side',cue:'Anchor at chest height, reach the working arm all the way across your body toward the anchor for a deep pre-stretch, then sweep it out and back — rear delt only, no shrugging',demo:'fly'}]},
      {id:'thu-chest',name:'Chest — Pull-Apart / Fly',pat:'fly',slot:'Push · Horizontal Push',target:'Chest · Rear Delts',equip:'Tube 10–20 → 40 lb',rx:'4×12–15',cal:35,cue:'Hug a big tree — slight elbow bend, feel the stretch open across your chest',demo:'fly',log:'setsreps',sets:4,
      ...
      {id:'mon-curl',name:'Bicep Curl',slot:'Pull · Biceps',target:'Biceps',equip:'Tube — stack toward your tested failure load',rx:'3×12–15',cal:25,cue:'Upper arms glued to your sides — only forearms move',log:'setsreps',sets:3,
      ...
      {id:'mon-tri',name:'Triceps Pushdown',slot:'Push · Triceps',target:'Triceps',equip:'Tube 10–20 → 50 lb',rx:'3×12–15',cal:25,cue:'Elbows pinned to ribs — only forearms move',log:'setsreps',sets:3,
  ...
  thu:{title:'Upper Body B',sub:'Push · Pull alternating — Chest · Back · Arms',accent:'var(--accent)',
    moves:[DS_WARMUP_RAISE,DS_WARMUP_ARMCIRCLE,DS_WARMUP_HIPFLOW9,DS_WARMUP_BANDSHOULDER,DS_WARMUP_SHOULDERCARS,
      {id:'mon-inclinepress',name:'Incline Close-Grip Press',slot:'Push · Chest',target:'Chest · Triceps · Front Delts',equip:'Tube 20–30 → 50 lb · low anchor',rx:'3×10–12',cal:30,cue:'Anchor low behind you, hands closer together than shoulder-width, elbows tucked in (not flared) — press up and out at an incline angle, squeeze the upper chest at the top. The narrow grip pulls real triceps work into a chest day on top of the upper chest/front delts — more muscle recruited per set than a wide-grip incline press.',demo:'press',log:'setsreps',sets:3,variants:[{name:'Standard-Grip Incline Press',equip:'Tube 20–30 → 50 lb · low anchor',rx:'3×10–12',cue:'Shoulder-width or slightly wider grip — same incline angle, shifts emphasis back toward pure chest with less triceps involvement',demo:'press'},{name:'Ball DB Chest Press',equip:'Ball + 2\u00d7 10 lb DBs',rx:'3\u00d712\u201315',cue:'Upper back on the ball, hips bridged up in line with your shoulders and knees (like a supported bench) \u2014 press both DBs straight up over your chest, lower until elbows are just below the ball line for a deep stretch, press back up. Keep hips locked \u2014 don\u2019t let them drop as you fatigue.',demo:'press'}]},
      {id:'thu-lat',name:'Underhand Pulldown',slot:'Pull · Vertical Pull',target:'Lats · Biceps · Forearms',equip:'Tube 20–30 → 50 lb',rx:'4–5×10–12',cal:35,cue:'Underhand (palms-up) grip — drive elbows down and into your back pockets, chest up, slight lean back. The underhand grip pulls real biceps/forearm work into a back day, on top of the lats — more muscle recruited per set than the standard overhand pulldown.',demo:'pulldown',log:'setsreps',sets:5,ramp:'1 set light band × 10, 1 set one step below working band × 6–8, then working sets.',
        variants:[{name:'Overhand Pulldown',equip:'Tube 20–30 → 50 lb',rx:'3–4×10–12',cue:'Standard palms-away grip — same pull pattern, shifts emphasis back toward pure lats with less biceps involvement',demo:'pulldown'},
                  {name:'Wide Row (free)',equip:'Tube 20–30 lb',rx:'3×12',cue:'Pull wide to the ribs, squeeze the mid-back',demo:'row'},
      ...
      {id:'mon-standbandpress',name:'Standing Band Chest Press',slot:'Push · Horizontal Push',target:'Chest',equip:'Tube 20–30 → 75 lb (Clench band) anchored behind back',rx:'3×12–15',cal:30,cue:'Band anchored around your back, thumbs looped through the handles — press straight out in front of you and squeeze the chest hard at full extension, then control the return',demo:'press',log:'setsreps',sets:3,variants:[{name:'Ball DB Chest Press',equip:'Ball + 2× 10 lb DBs',rx:'3×12–15',cue:'Upper back on the ball, hips bridged level with shoulders and knees — press both DBs up over the chest, lower to a deep stretch just below the ball line, press back up. Keep hips locked, no sagging as you fatigue',demo:'press'}]},
      {id:'mon-pullapart',name:'Band Pull-Apart',slot:'Pull · Rear Delts',target:'Rear Delts',equip:'Tube 10–20 → 50 lb',rx:'3×15–20',cal:25,cue:'Crack a walnut between your shoulder blades — arms stay straight',demo:'fly',log:'setsreps',sets:3,variants:[{name:'Cross-Body Rear Delt Fly',equip:'Tube 10–20 lb · chest-height anchor',rx:'3×12–15/side',cue:'Anchor at chest height, reach the working arm all the way across your body toward the anchor for a deep pre-stretch, then sweep it out and back — rear delt only, no shrugging',demo:'fly'}]},
      {id:'thu-lateral',name:'Lateral Raise',demo:'lateralraise',slot:'Push · Side Delts',target:'Side Delts',equip:'Tube 10 → 30 lb',rx:'4×12–15',cal:25,cue:'Lead with elbows, not hands — pour water from a pitcher',log:'setsreps',sets:4,
        variants:[{name:'Front-Angled Lateral Raise',equip:'Tube 10 lb · anchored low in front of you',rx:'3×12–15',cue:'Stand on the band so it pulls slightly from in front rather than straight down — this loads the side delt earlier, right at the stretched bottom position',demo:'lateralraise'},{name:'Incline Ball Lateral Raise',equip:'Ball + 2× 10 lb dumbbells (or 2 lb to start)',rx:'3×12–15',cue:'Lie chest-down on the ball at an incline, feet braced on the floor behind you — raise both DBs out to your sides leading with the elbows, control the lower for a deep stretch at the bottom. The ball locks your torso still so the delt does all the work instead of momentum.',demo:'lateralraise'}]},
      {id:'thu-shrug',name:'Banded Shrug',slot:'Pull · Traps',target:'Traps (upper)',equip:'Tube 20–30 lb · stand on band',rx:'3×15–20',cal:20,cue:'Straight up and down, no rolling — pause 1 sec at the top, control the lower',demo:'trap3',log:'setsreps',sets:3,
      ...
      {id:'thu-ohtriceps',name:'Banded Overhead Triceps Extension',slot:'Push · Triceps',target:'Triceps — Long Head',equip:'Tube 10–20 lb, anchored underfoot',rx:'3×10–12',cal:20,cue:'Added for a second triceps angle this week — pushdowns train elbow extension with arms at your sides, this trains it with the triceps in a stretched overhead position, which pushdowns miss. Anchor the band under one foot, hold both ends overhead — lower behind the head by bending only the elbows, press back to lockout. Elbows stay close to your ears, no flaring out.',demo:'triceps',log:'setsreps',sets:3,
  ...
  fri:{title:'Lower Body Pull',sub:'Hamstrings · Glutes · Lower Back',accent:'var(--accent)',
    moves:[DS_WARMUP_RAISE,DS_WARMUP_ARMCIRCLE,DS_WARMUP_HIPFLOW7,DS_WARMUP_HIPCARS,
      ...
      {id:'fri-slrdl',name:'Romanian Deadlift (Bilateral)',slot:'Hinge',target:'Hamstrings',equip:'Tube 40–50 → 90+ lb',rx:'3–4×8–12',cal:35,cue:'Stand on the tube, soft knees, push hips straight back — flat back, handles tracing down the thighs to a deep stretch, drive hips forward to finish. Pause 2 full counts at the bottom before driving back up — the dead-stop kills momentum and keeps tension on the hamstrings instead of bouncing out of the stretch. Look forward, not down — down pulls the head and upper back into flexion even when the hips are doing everything right; a forward gaze is what actually keeps the whole spine in its neutral curve. Now your primary Friday hinge — stack bands and push to true failure the same way you\'re doing with rows/curls/pulldown.',demo:'hinge',log:'setsreps',sets:4,ramp:'1 set light band × 10, 1 set one step below working band × 6–8, then working sets. Extra important once you\'re on the heavier bands, since spinal loading goes up with them.',
        ...
        setup:'Stand on the center of the band with both feet, soft bend in the knees. Push your hips straight back, letting the handles trace down the front of your thighs with a flat back, until you feel a deep hamstring stretch — pause 2 counts here — then drive the hips forward to stand tall. Keep your gaze forward the whole rep, not down at the floor.'},
...
function dsPickDay(d){
  if(d==='extras'){ DS_EXTRAS_ACTIVE=!DS_EXTRAS_ACTIVE; dsRender(); return; }
  DS_EXTRAS_ACTIVE=false;
  DS_DAY_OVERRIDE=(d===dsRealSessionKey(activeDate))?null:d; dsRender();
}
function dsBackToRealDay(){ DS_DAY_OVERRIDE=null; DS_EXTRAS_ACTIVE=false; dsRender(); }
...
var DS_PULLUP={key:"pullup",title:"Pull-Up Progression",accent:"#7dd3fc",meta:"toward your first rep \u00b7 alternating days",blurb:"Your road to the first unassisted pull-up \u2014 a 10-stage ladder from Dead Hang to strict Bodyweight Pull-ups. Log each drill so the numbers climb \u2014 that climb is the progress. Hit the milestone rep/hold target for 3 sets before advancing to the next stage. Take the hardest drill close to failure; ease off if the elbow flares.",moves:[
 ...
 {id:"pu-hang",name:"1. Dead Hang \u2014 log seconds",rx:"3\u00d715s (build toward)",cal:10,demo:"deadhang",log:"setsreps",sets:3,target:"Grip \u00b7 Shoulders \u00b7 Lats",equip:"Monkey bars",cue:"Shoulders active, pulled down away from your ears \u2014 don't just dangle. Milestone: 15s hold \u00d7 3 sets, then move to Scapular Rows",setup:"Hang from the bar, full grip, arms straight, shoulder blades pulled down and back. Put the seconds you held into the Reps box and beat last time. Once you can hold 15 seconds for all 3 sets, you're ready for Scapular Rows."},
 {id:"pu-scaprow",name:"2. Scapular Rows",rx:"3\u00d710",cal:10,target:"Lower Traps \u00b7 Lats \u00b7 Rhomboids",equip:"Low bar, TRX-style strap, or sturdy table edge",log:"setsreps",sets:3,cue:"Feet planted, body angled back, straight line head-to-heel \u2014 row your chest toward the bar without bending elbows much, driving through the shoulder blades",setup:"Grip a low bar or strap, walk your feet forward so your body leans back at an angle with arms extended. Keeping arms mostly straight, pull your shoulder blades down and together to row your chest up toward the bar, then release with control. Milestone: 10 reps \u00d7 3 sets, then move to Scapular Pull-ups."},
 {id:"pu-scap",name:"3. Scapular Pull-ups",rx:"3\u00d76\u20138",cal:10,demo:"scappull",log:"setsreps",sets:3,target:"Lower Traps \u00b7 Lats",equip:"Monkey bars",cue:"Arms stay straight \u2014 the shoulder blades do all the work. As you pull down, imagine trying to twist a wooden twig in your hands and snap it \u2014 down AND slightly inward, not just straight down",setup:"Hang with straight arms. Without bending the elbows, pull the shoulder blades down to lift your chest a couple inches, then lower with control. Milestone: 6\u20138 reps \u00d7 3 sets, then move to Australian Pull-ups (Inverted Rows)."},
 {id:"pu-aussie",name:"4. Australian Pull-ups (Inverted Row)",rx:"3\u00d76\u20138",cal:15,demo:"row",log:"setsreps",sets:3,target:"Back \u00b7 Biceps \u00b7 Full Pulling Chain",equip:"Low bar or sturdy table edge",cue:"Body straight as a plank, pull your chest to the bar leading with the elbows \u2014 the more horizontal your body, the harder it is, so start more upright if needed",setup:"Lie under a low bar with the bar at chest height, grip it, and hold your body in a straight line with heels on the ground. Pull your chest up to the bar, squeeze, then lower under control. Adjust foot position \u2014 feet further out (more horizontal body) is harder. Milestone: 6\u20138 reps \u00d7 3 sets, then move to Jackknife Pull-ups."},
 {id:"pu-jackknife",name:"5. Jackknife Pull-ups",rx:"3\u00d76\u20138",cal:15,demo:"pullneg",log:"setsreps",sets:3,target:"Full Pulling Chain",equip:"Monkey bars + box or chair",cue:"Push off the box just enough to assist the pull \u2014 let the arms and back do as much of the work as they can",setup:"Stand on a box or chair positioned under the bar with knees bent. Use your legs to push off and assist as you pull your chin over the bar, then lower slowly. Reduce leg drive over time as pulling strength builds. Milestone: 6\u20138 reps \u00d7 3 sets, then move to Jackknife Top Hold."},
 {id:"pu-jacktop",name:"6. Jackknife Top Hold",rx:"3\u00d710s hold",cal:15,demo:"scappull",log:"setsreps",sets:3,target:"Full Pulling Chain \u00b7 Isometric Strength",equip:"Monkey bars + box or chair",cue:"Push off just enough to get your chin over the bar, then hold there as long as you can \u2014 fight to keep the chin up",setup:"Same setup as Jackknife Pull-ups \u2014 push off the box to get your chin over the bar, then remove the leg assist and hold the top position isometrically. Milestone: 10-second hold \u00d7 3 sets, then move to Band-Assisted Pull-ups."},
 {id:"pu-band",name:"7. Band-Assisted Pull-ups",rx:"3\u00d76\u20138",cal:15,demo:"pullband",log:"setsreps",sets:3,target:"Full pulling chain",equip:"Clench 35\u201375 lb band over the bar (step down to a medium tube band as you get stronger)",cue:"Elbows drive down and back, chest toward the bar \u2014 twist the bar like you're breaking a twig (down and in) so the lats fire first, not the biceps",setup:"Loop the Clench band over the bar, foot or knee in the loop. This band gives more assistance than your tube bands, so it's the right starting point for this stage \u2014 do full pull-ups, logging reps and which band in the weight box. Don't just haul straight up \u2014 pull down and slightly inward from the very first inch, like snapping a wooden twig rather than an iron bar. Milestone: 6\u20138 reps \u00d7 3 sets, then either drop to a medium tube band for less assistance at the same rep target, or move on to Band-Assisted Top Hold."},
 {id:"pu-bandtop",name:"8. Band-Assisted Top Hold",rx:"3\u00d710s hold",cal:15,demo:"pullband",log:"setsreps",sets:3,target:"Full pulling chain \u00b7 Isometric Strength",equip:"Clench 35\u201375 lb band over the bar (step down to a medium tube band as you get stronger)",cue:"Pull to the top with band assist, then hold your chin over the bar as long as you can before lowering",setup:"Same band setup as Band-Assisted Pull-ups \u2014 pull to the top and hold the chin-over-bar position isometrically instead of doing continuous reps. Milestone: 10-second hold \u00d7 3 sets, then step down to a lighter band for the same hold before moving to Negatives."},
 {id:"pu-neg",name:"9. Negatives \u2014 log slow seconds",rx:"3\u00d75 slow reps",cal:15,demo:"pullneg",log:"setsreps",sets:3,target:"Full pulling chain",equip:"Monkey bars",cue:"Fight gravity the whole way \u2014 slower is the whole point. On the way down, try to twist the bar apart in your hands (like snapping a twig, not an iron bar) \u2014 that inward torque keeps the lats loaded instead of letting the biceps take over",setup:"Jump or step to the top with your chin over the bar, then lower as slowly as you can for all 5 reps. Log the seconds the slow part lasted in Reps. Milestone: 5 slow eccentric reps \u00d7 3 sets, then move to full Bodyweight Pull-ups."},
...
var DS_MV={
  ...
  'thu-lat':{'Back':1,'Biceps':.7,'Forearms':.5},
...
function dsRender(){
  ...
  var eb=document.getElementById('ds-eyebrow'); if(eb)eb.textContent=DS_EXTRAS_ACTIVE?'Optional extras':((activeDate===todayKey()?'Today':'Selected day')+' \u00b7 '+DS_DAYLABEL[sk]);
  var tt=document.getElementById('ds-title'); if(tt)tt.textContent=DS_EXTRAS_ACTIVE?'Extras':SS.title;
  var sb=document.getElementById('ds-sub'); if(sb)sb.textContent=DS_EXTRAS_ACTIVE?'Not day-specific \u2014 stack any of these on top of your regular session':SS.sub;
